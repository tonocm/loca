...
static stamp_tt my_count[MAX_WINDOW][2];
...
VOID Fini(INT32 code, VOID *v){
  ...
  for(i=1;i<=sublog_value_to_index<MAX_WINDOW, SUBLOG_BITS>(N);i++){
    ...
    sum += my_count[i][0] + first_count[i][0];
    mr -= my_count[i][0] * 1.0 / N;
    ResultFile << i << "\t" << ws << "\t" << fp*(1<<WORDSHIFT1) << "\t" <<  my_count[i][0]*1.0/N << "\t" << mr << "\t";

    fp2 = ( 1.0 * (( N - sum2 ) * ws + sum_i2)) / (N - ws + 1);
    sum_i2 += count_i[i][1] + first_count_i[i][1];
    sum2 += my_count[i][1] + first_count[i][1];
    mr2 -= my_count[i][1] * 1.0 / N;
    ResultFile << fp2*(1<<WORDSHIFT2) << "\t" <<  my_count[i][1] * 1.0 / N << "\t" << mr2 << endl;
  }
  ResultFile.close();  
...
int main(int argc, char *argv[])
{
    ...
    memset (my_count, 0, MAX_WINDOW*sizeof(stamp_tt)*2);
